...
void Keeper_compute(struct data *dataRobot, int *dir, int *vel, int *orient, bool back, bool toCenter) {
    ...
    if(back == true) {  
        ...
        *orient = 0; 
        ...
        keeper_vectY_lines(*dataRobot, true);   // reset keeper out of lines  
    }
    ...
    *vel = velocity;
    ...
    static unsigned int pause = PAUSA_SPAZZATA_CENTRO; 
...
int keeper_vectY_lines(struct data dataRobot, bool resetOUTlines)  {
    ...
    // flag reset
    if((OI || OE || EI || EE || NI || NE) || resetOUTlines) flag_out = false; 
...
int orientFilter(int ang) {
  ...
  if(((ang > 180) && (ang_prec < 180))  ||  (((ang < 180) && (ang_prec > 180)))) {
    ...
    angFilter = (ang * COEFF_ORIENT_KEEPER) + (ang_prec * (1 - COEFF_ORIENT_KEEPER));
  }
  else {
    angFilter = (ang * COEFF_ORIENT_KEEPER) + (ang_prec * (1 - COEFF_ORIENT_KEEPER));
  }
