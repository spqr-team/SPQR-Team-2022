...
// array soglie individuali per ogni sensore 
unsigned long thresholds[8] = {0};
...
byte readLines_current() {
  ...
  for (byte i = 0; i < 8; i++) {
    ...
    // autotaratura soglie individuali 
    if (analogRead(linesensAll[i]) > thresholds[i]) Sread |= (1 << i);
  }
...
// taratuta automatica soglia per ogni sensore
void init_lines() {

  unsigned long soglia = 0; 
  
  // per ogni sensore, media letture
  for(byte i = 0; i < 8; i++) {

    soglia = 0; 

    for(long reading = 0; reading < LETTURE; reading++)  {
      soglia += analogRead(linesensAll[i]); 
      delay(3);
    }

    soglia = (soglia / LETTURE) + 70; 
    thresholds[i] = soglia; 
...
void lineSensorsTest() {
  ...
  for (byte i = 0; i < 8; i++) {
    if (analogRead(linesensAll[i]) > thresholds[i]) Stotal |= (1 << i);
  }
