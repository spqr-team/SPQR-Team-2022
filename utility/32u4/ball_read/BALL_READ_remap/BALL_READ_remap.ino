// ball read 32u4 su nuova scheda, perchè i sensori montati sopra vedono di meno 
// in quanto sono coperti dal teensy 
// la dispozione dei sensori è quella di rick del 2020
...
void loop() { 
  ...
  int angleI = ((int)(angle + 360)) % 360;

  // angle correction-remapping 
  int err = 0; 
  if(angleI > 0 && angleI <= 110) err = map(angleI, 0, 110, 0, 20);
  if(angleI > 110 && angleI <= 180) err = map(angleI, 110, 180, 20, 0);
  if(angleI > 180 && angleI <= 250) err = map(angleI, 180, 250, 0, -20);
  if(angleI > 250 && angleI <= 360) err = map(angleI, 250, 360, -20, 0);
